...
//Serial1.print(ac);
 ...
 while (true)  
  ...
  void bluetooth();                                                     // Run the Bluetooth procedure to see if there is any data being sent via BT
  ...
  void getGPS();                                                        // Tiny GPS function that retrieves GPS data - update GPS location// delay time changed from 100 to 10
   ...
   */ 
    if (Distance_To_Home == 0)                                   // If the Vehicle has reached it's Destination, then Stop
        {
        void StopCar();                                               // Stop the robot after each waypoint is reached
